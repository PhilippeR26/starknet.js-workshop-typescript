...
async function main() {
    ...
    const response = await axios.post("http://192.168.1.11:9545/rpc/pathfinder/v0.1", payload_Pathfinder);
    ...
    const compiledSierra = json.parse(fs.readFileSync("./compiledContracts/cairo240/merkle_verify_pedersen.sierra.json").toString("ascii"));
    const compiledCasm = json.parse(fs.readFileSync("./compiledContracts/cairo240/merkle_verify_pedersen.casm.json").toString("ascii"));
    
    const estimateDecl = await account0.estimateDeclareFee({ contract: compiledSierra, casm: compiledCasm });
    console.log(estimateDecl, "\n", Number(formatBalance(estimateDecl.suggestedMaxFee, 18)) * 2500, "US$");
    // process.exit(5);
