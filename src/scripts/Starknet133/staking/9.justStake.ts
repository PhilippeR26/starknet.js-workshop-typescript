// Initialize a validator staker
// Launch with npx src/scripts/Starknet133/staking/8.partialPoolChange.ts
...
async function main() {
  ...
  const balance0 = await strkContract.balanceOf(accountTest2.address);
  console.log("Initial balance account2 =", formatBalance(balance0, 18), "STRK");
  ...
  const stakeCall = stakingContract.populate("stake", {
    reward_address: accountTest2.address,
    operational_address: accountTest2.address,
    amount: qty * 10n ** 18n,
    pool_enabled: false,
    commission: 0,
  });
  console.log("🔜 stake...");
  const resp0 = await accountTest2.execute([approveCall1, stakeCall]);
  const txR0 = await accountTest2.waitForTransaction(resp0.transaction_hash);
  if (txR0.isSuccess()) { console.log("Success.") } else { console.log("Error 0", txR0); process.exit() }

  const info0: CairoOption<StakerInfo> = await stakingContract.get_staker_info(accountTest2.address);
  if (info0.isSome()) {
    const info = info0.unwrap() as StakerInfo;
    console.log(info);
    console.log("amount stacked =", formatBalance(BigInt(info.amount_own), 18), "STRK");
  }
  else { console.log("No staker"); }

  const balance1 = await strkContract.balanceOf(accountTest2.address);
  console.log("Final balance account2 =", formatBalance(balance1, 18), "STRK");
