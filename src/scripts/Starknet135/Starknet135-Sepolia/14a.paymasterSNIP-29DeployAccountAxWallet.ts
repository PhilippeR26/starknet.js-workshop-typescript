// Deploy an AX account with SNIP-29 paymaster.
...
async function main() {
  ...
  const resp0 = await account0.execute({ contractAddress: gasToken, entrypoint: "transfer", calldata: [AXcontractAddress, 1n * 10n ** 5n, 0n] });
  const txR = await account0.waitForTransaction(resp0.transaction_hash);
  // console.log("txR", txR);

  // *** simulate the response of a wallet: 
  // equiv to: const deploymentData = wallet.deploymentData(StarknetWalletObject)
  ...
  const newAccount = new Account(myProvider, AXcontractAddress, privateKey, undefined, undefined, paymasterRpc);
