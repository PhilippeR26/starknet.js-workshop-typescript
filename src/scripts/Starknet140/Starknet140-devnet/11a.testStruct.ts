...
async function main() {
    ...
    const myTypePoint = new CairoStruct(myPoint, abiPoint, strategies);
    const comp0 = myTestCallData.compile("struct_point", [myPoint]);
    const comp0a = myTestCallData.compile("struct_point", { x: myPoint });
    const comp0b = myTestCallData.compile("struct_point", [myTypePoint]);
    const comp0c = myTestCallData.compile("struct_point", { x: myTypePoint });
    console.log("struct_point comp0 compile() =", comp0);
    console.log("struct_point comp0a compile() =", comp0a);
    console.log("struct_point comp0b compile() =", comp0b);
    console.log("struct_point comp0c compile() =", comp0c);
    const res0a = (await myTestContract.call("struct_point", [myPoint])) as Point;
    const res0b = (await myTestContract.call("struct_point", [myTypePoint])) as Point;
    const res0 = (await myTestContract.struct_point(myPoint)) as Point;
    const res0c = (await myTestContract.struct_point(myTypePoint)) as Point;
    console.log("res0 =", res0);
    console.log("res0a =", res0a);
    console.log("res0b =", res0b);
    console.log("res0c =", res0c);
    const res0d = CallData.compile([myPoint]);
    const res0e = CallData.compile({ x: myPoint });
    const res0f = CallData.compile([myTypePoint]);
    const res0g = CallData.compile({ x: myTypePoint });
    ...
    const myCall8 = myTestContract.populate("write_struct_point", { x: op8 });
    const myCall8b = myTestContract.populate("write_struct_point", [op8]);
