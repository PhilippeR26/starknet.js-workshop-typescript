...
async function main() {
  ...
  const account0Proof = new Account({ provider: myProvider, address: accountAddress0, signer: privateKey0 });
  ...
  const estimModified = { 
    ...estim.resourceBounds, 
    l2_gas: { ...estim.resourceBounds.l2_gas, max_amount: 100000000n } 
  } as ResourceBoundsBN;
  ...
  fs.writeFileSync('./src/scripts/Starknet142/Starknet142-Mainnet/proofResultM.json', json.stringify({ proofRes, messageFromProof }, undefined, 2));
