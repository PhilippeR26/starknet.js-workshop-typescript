// Test the secret proof contract in Mainnet, with an existing account and a pre-deployed contract (deployed in the previous script 20m)
// Launch with npx ts-node src/scripts/Starknet142/Starknet142-Sepolia/21m.testSecretProof.ts
...
async function main() {
  ...
  const myProviderProof = new RpcProviderProof({ nodeUrl: "https://starknet-mainnet.g.alchemy.com/starknet/version/rpc/v0_10/" + alchemyKey }); // Sepolia Testnet 
  ...
  // Connect the new contract instance (deployed in Mainnet) :
  ...
  const estim = await account0Proof.estimateInvokeFee(myCalldata);
  console.log("estim:", estim);
  const estimModified = { ...estim.resourceBounds, l2_gas: { max_amount: 100000000n, max_price_per_unit: estim.resourceBounds.l2_gas.max_price_per_unit } } as ResourceBoundsBN;
  console.log("estim modified:", estimModified);
  const tx = await account0Proof.getSignedTransaction(myCalldata, { resourceBounds: estimModified });
  console.log(tx);
  ...
  fs.writeFileSync('./src/scripts/Starknet142/Starknet142-Sepolia/proofResultM.json', json.stringify({ proofRes, messageFromProof }, undefined, 2));
