// STRK20 shield/unshield WITHOUT a wallet — SEPOLIA version, on a SELF-DEPLOYED pool.
// Same pipeline as the Mainnet PoC (1.strk20ShieldUnshield.ts in Starknet143-Mainnet):
...
//     implementation verified against the official golden vectors at startup).
//
// Proofs are the expensive part (~2.3 STRK gas each), so actions are BATCHED: tx 1/2 =
// onboarding (register/channel/subchannel, as needed) + SHIELD in one proof; tx 2/2 =
// UNSHIELD. Two proofs is the minimum for a round-trip — use-notes(4) must precede
// create-notes(5), so a note can't be spent in the same tx that creates it.
//
// launch with : npx ts-node src/scripts/Starknet143/Starknet143-Sepolia/5.strk20ShieldUnshield.ts
...
// pool address = 0xc7784a608eec3a1b933fe7f2630440188d61b3bbe28b597c76a06fd3686353
// ================== constants to adapt ==================
...
async function main() {
    ...
    // so onboarding (register/channel/subchannel) is visible to the deposit that follows.
    ...
    // attestation for the deposit). Only the UNSHIELD needs a 2nd proof: use-notes(4) must
    // precede create-notes(5), so a note cannot be spent in the tx that creates it.
