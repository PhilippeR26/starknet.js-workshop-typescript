// Use a Ledger Nano S+/X Starknet APP 2.2.3 to sign a transaction.
// Use of a Starknet.js signer
// Launch with npx ts-node src/scripts/ledgerNano/8.testLedgerAccount223.ts
// Coded with Starknet.js v6.23.1 + experimental & devnet-rs v0.2.4 & starknet-devnet.js v0.2.2
...
async function main() {
  ...
  const myCall = EthContract.populate("transfer", [account0.address, 2n * 10n ** 12n]);
  ...
  const resV1b = await ledgerAccount.execute([myCall, myCall, myCall]);
  ...
  console.log("Processing 3 calls...");
  const resV3b = await ledgerAccount.execute([myCall, myCall, myCall], { version: 3 });
