...
async function main() {
  ...
  console.log("Declare...");
  const accountOZSierra = json.parse(fs.readFileSync("./compiledContracts/cairo264/openZeppelin14/openzeppelin_AccountUpgradeable.sierra.json").toString("ascii"));
  const accountOZCasm = json.parse(fs.readFileSync("./compiledContracts/cairo264/openZeppelin14/openzeppelin_AccountUpgradeable.casm.json").toString("ascii"));
  ...
  const ledgerAccount = new Account(myProvider, ledger0addr, myLedgerSigner, undefined, ETransactionVersion.V2);
