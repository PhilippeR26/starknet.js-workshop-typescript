...
// launch with npx ts-node src/scripts/merkleTree/airdropSJS6Mainnet/1.createTreeMainnet.ts
...
async function main() {
    const list = json.parse(fs.readFileSync("./src/scripts/merkleTree/airdropSJS6Mainnet/listAddressesMainnet.json").toString("ascii"));
    ...
    fs.writeFileSync('./src/scripts/merkleTree/airdropSJS6Mainnet/treeListAddressMainnet.json', JSON.stringify(tree1.dump(),undefined,2));

    const tree = Merkle.StarknetMerkleTree.load(
        JSON.parse(fs.readFileSync('./src/scripts/merkleTree/airdropSJS6Mainnet/treeListAddressMainnet.json', 'ascii'))
    );
    // process.exit(5);
    const accountAddress = '0x07Fdcad2A136889F14E1516733c512642bDCFd5309e2D5c7B4d5120D21Afa608';
