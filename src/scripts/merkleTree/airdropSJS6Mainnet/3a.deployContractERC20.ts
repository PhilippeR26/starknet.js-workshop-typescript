// Deploy class.
// launch with npx ts-node src/scripts/merkleTree/airdropSJS6Mainnet/3a.deployContractERC20.ts
// Coded with Starknet.js v6.0.0 B13

import { constants, Contract, Account, json, shortString, RpcProvider, RPC, num, CallData } from "starknet";
import fs from "fs";
...
async function main() {
    ...
    const contractClassHash = "0x779f8e128064c8663491f842e11970ca799620cbdc5d5600a312621cabf3c67";
    ...
    const estimateDeploy = await account0.estimateDeployFee({ classHash: contractClassHash, constructorCalldata });
    console.log(estimateDeploy, "\n", Number(formatBalance(estimateDeploy.suggestedMaxFee, 18)) * 2300, "US$");
    process.exit(5); // remove once fee accepted
    console.log("Processing...");
    const declResp = await account0.deployContract({ classHash: contractClassHash, constructorCalldata });
    ...
    console.log(txR);
    //}
    console.log("Contract address =", declResp.address);
