import { BigNumberish, shortString, num, byteArray, RpcProvider, Account, json, Contract, CallData, parseCalldataField, type Abi } from "starknet";
...
async function main() {
    ...
    console.log(num.toHex("2"));
    const param = byteArray.byteArrayFromString("Take care.");
    const selectedType = "core::byte_array::ByteArray";
    const { abi } = json.parse(fs.readFileSync("./compiledContracts/cairo260/string.sierra.json").toString("ascii"));
    const myCallData=new CallData(abi);
    const myCalldata=myCallData.compile("proceed_string",["ezrtezrtzertzet"]);
    ...
    const abiExtract = abi.find((abiItem: any) => abiItem.name === selectedType);
    console.log("abiExtract=", abiExtract);
    const inputAbi = { name: abiExtract.type, type: abiExtract.name };
